...
<form method="POST" action="{{ route('register') }}" id="registrationForm">
    ...
    <div class="step-content" id="step3">
        ...
        <div class="form-group">
            <label class="form-label" style="margin-bottom: 0.5rem; display:block;">Participant Type <span style="color: var(--danger);">*</span></label>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                @foreach([
                ...
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="participant_type" value="{{ $type }}" required {{ old('participant_type') == $type ? 'checked' : '' }}>
                    {{ $type }}
                </label>
                @endforeach
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="participant_type" value="Other" id="participantTypeOtherRadio" required {{ old('participant_type') == 'Other' ? 'checked' : '' }}>
                    Other
                </label>
            </div>
            <input type="text" id="participantTypeOtherInput" class="form-input" placeholder="Please specify..." style="display:none; margin-top: 0.5rem;">
            @error('participant_type') <div class="form-error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" style="margin-bottom: 0.5rem; display:block;">Ministry / Department / Agency <span style="color: var(--danger);">*</span></label>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                @foreach([
                ...
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="ministry_agency" value="{{ $agency }}" required {{ old('ministry_agency') == $agency ? 'checked' : '' }}>
                    {{ $agency }}
                </label>
                @endforeach
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="radio" name="ministry_agency" value="Other" id="ministryOtherRadio" required {{ old('ministry_agency') == 'Other' ? 'checked' : '' }}>
                    Other
                </label>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('input[type="text"].form-input, textarea.form-textarea').forEach(function(input) {
    ...
    // Setup "Other" radio handlers
    function setupOtherRadio(radioName, otherRadioId, otherInputId) {
    ...
    setupOtherRadio('salutation', 'salutationOtherRadio', 'salutationOtherInput');
    setupOtherRadio('participant_type', 'participantTypeOtherRadio', 'participantTypeOtherInput');
    setupOtherRadio('ministry_agency', 'ministryOtherRadio', 'ministry_agency_other');
