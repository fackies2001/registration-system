...
class ApprovalLoginLinkNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        $message = (new MailMessage())
            ...
            ->line('Your registration has been approved and you now have full access to the system.');

        if ($notifiable->approval_note) {
            $message->line('Note from Admin: ' . $notifiable->approval_note);
        }

        if ($notifiable->meeting_link) {
            $message->line('Please use the following link to join the meeting:');
            $message->line($notifiable->meeting_link);
        }
